...
// icx -o build/t_selection algorithms/sort/selection.c test/algorithms/sort/t_selection.c -I./algorithms/sort/ -flto -O3 ; ./build/t_selection
...
int main()
{
	...
	print_arr(arr, cnt < 20 ? cnt : 20);
	struct sortable_array array = { .count = cnt,
					.data = arr,
					.data_size = 4 };

	start = clock(); // 记录开始时间
	merge_sort(array, comp);
	end = clock(); // 记录结束时间
